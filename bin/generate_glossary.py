...
def get_markdown_title_and_content(file_path: str) -> tuple[str | None, str]:
    ...
    title = None
    content_lines = []
    first_title_found = False  # Flag para verificar se o primeiro título foi encontrado

    with open(file_path, "r", encoding="utf-8") as file:
        for line in file:
            if line.startswith("# "):
                title = line[2:].strip()  # Remove o '# ' e os espaços em branco
...
def increment_title_levels(content: str) -> str:
    ...
    updated_content = re.sub(r'^(#+)\s+(.*)', replace_title, content, flags=re.MULTILINE)
...
def generate_glossary_content() -> str:
    ...
    files = [f.strip() for f in os.listdir(docs_dir) if f.lower().endswith(".md")]
    glossary_entries = []
    ...
    for file_name in files:
        ...
        if title:
            ...
            content = increment_title_levels(content)
...
def update_readme_with_glossary() -> None:
    glossary_content = generate_glossary_content()
